                        ...
                        <div class="card m-b-30">
                            <div class="card-body">
                                <form action="" id="admins" method="POST" data-parsley-validate>
                                    ...
                                    <input type="hidden" name="adminId" id="admin_id" value="{{ $admin->id }}"
                                        class="form-control">
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-10">
                                            <input class="form-control" type="text" name="name" id="example-text-input"
                                                placeholder="{{ __('admin.name') }}" value="{{ $admin->name }}">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-10">
                                            <input class="form-control" type="email" name="email" id="example-text-input"
                                                placeholder="{{ __('admin.email') }}" value="{{ $admin->email }}">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-10">
                                            <select class="form-control form-control-round" name="role_id" id="role_id">
                                                <option value="">---</option>
                                                @foreach ($roles as $role)
                                                    <option value="{{ $role->id }}"
                                                        {{ $admin->hasRole($role->name) == 'true' ? 'selected' : '' }}>
                                                        {{ $role->name }}</option>
                                                @endforeach
                                            </select>
                                            <span class="text-danger" id="role_idError"></span>
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        <label class="col-sm-2 col-form-label">@lang('admin.department')</label>
                                        <div class="col-sm-10">
                                            <select class="form-control form-control-round" name="department_id" id="department_id">
                                                <option value="{{ $admin->department_id }}">{{$admin->departments->name}}</option>
                                                @foreach ($departments as $department)
                                                    <option value="{{ $department->id }}"
                                                        {{ $admin->hasRole($department->name) == 'true' ? 'selected' : '' }}>
                ...
                $('body').on('submit','#admins',function (e) {
                    ...
                    $.ajax({
                        ...
                        error:function (response) {
                            ...
                            $('#role_idError').text(response.responseJSON.errors.role_id);
                            $('#department_idError').text(response.responseJSON.errors.department_id);
